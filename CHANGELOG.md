...
- `devctl test` runs integration tests against a throwaway stack. It starts a [named instance](docs/parallel-stacks.md#named-instances) of its own, waits until it is healthy, and runs a task or a `-- command`. The command gets the stack's ports as `DEVCTL_<SERVICE>_<PORT>_PORT`, plus `DEVCTL_PROXY_URL` and, with `--env-from <service>`, that service's resolved environment. On failure it writes a bundle to `--artifacts`. It then tears the stack down, leaving nothing running, and exits with the command's code, or 5 or 6 if the stack failed. `devctl start --wait [--timeout 5m]` blocks until every started service is ready, and exits 6 naming the services that are not. `devctl bundle [--since 10m] [--output file.tgz]` writes the same bundle on demand: status, logs, traces of failed requests, traffic, doctor, config diff, bootstrap log and versions. Every file is redacted, including literal secret values from the config. See [Tests and CI](docs/ci.md), which has a GitHub Actions example. Experimental. (#118)
- A [roadmap](docs/roadmap.md) with Phases 1–3 before v1.0 and their issues, linked from [Platform bets](docs/platform-bets.md) and the README. Features that may still change without a deprecation period carry one **Experimental** line in their docs, listed in [Experimental features](docs/roadmap.md#experimental-features): `transform.request_body`, `auth.suppress_authorization`, LLM `capture.field_map` and `cost_per_token`, gRPC body decoding, `environment.sops`, `environment.terraform`, and parallel stacks. Graduations will be noted here. A "Doesn't fit my stack" issue template collects onboarding reports from other stacks. (#115)
...
- Parallel stacks: several checkouts of one configuration, such as git worktrees for parallel coding agents, run at the same time without editing ports. The first checkout to start takes port slot 0 and keeps its configured ports. Each other checkout takes the next free slot, and every fixed service port and listener (proxy, token endpoint, gRPC routes, OTLP receiver, web console) moves up by 100 per slot. `SERVICE_PORT`, `<NAME>_PORT`, `${services.…}` references and a health check on the service's own loopback port follow the shift. A slot stays with its checkout until a full `devctl down`. `devctl instances` lists the slots with their checkout, ports and status, and `devctl instances prune` stops and frees the stacks of deleted checkouts. `devctl status` shows the slot. Named instances, per-instance volumes and per-worktree MCP snippets are still to come. See [Parallel stacks](docs/parallel-stacks.md). (#117)
...
- The demo-platform integration test (`supervisor.integration.test.ts`) moves the identity service to a free port. It no longer fails with `identity blocked: … is using port 18001` while the demo is running locally.
- Web overview **Failed requests** counts proxy failures (HTTP 4xx/5xx or a proxy error) instead of ERROR log lines. Graph splits that into requests versus failed requests, and a separate logs versus ERROR/FATAL chart. Recent errors on the overview stays the log stream. See [Web console](docs/web.md#explore-the-dependency-graph).
...
- Freeing a busy port no longer risks stopping the wrong process. Doctor's free-port action used the process it found when the row was drawn, and sent SIGTERM and then SIGKILL to that pid without checking that it still held the port. If that process had exited and its pid been reused, or it let go of the port on SIGTERM, an unrelated process was signalled. Now devctl looks up the port again before each signal, stops nothing when another process holds it, and reports a port that is already free. When a service stops, devctl frees its port only if the holder is provably that service's process: same command, plus a matching working directory or start time. A name match alone no longer counts, so on Windows, where `tasklist` reports only the program name, an unrelated process of the same program is left running. For a service that a reload removed, the holder is matched against the process devctl last recorded for it. See [Doctor](docs/doctor.md). (#145)
- When the proxy or token endpoint fails to bind, stopping it no longer tries to close a listener that never opened. On Windows that crashed Bun (`Segmentation fault at address 0x8`), which showed up in CI in the test for the proxy bind fix.
- On Windows, services keep running after the supervisor exits, as on macOS and Linux. They were not spawned detached there, so they ended with the supervisor: `devctl down --keep-services` had to leave the supervisor process running until they exited, and a supervisor crash took them down instead of leaving them for the next start to adopt. Services now start detached on every platform. On Windows they also start with no console window. `down --keep-services` ends the supervisor everywhere.
- The port-holder lookup no longer reports the port number as the holding process. When `lsof` found no listener, devctl fell back to `fuser` and took the first number from its combined output. If `fuser` could not list the holder (a process owned by another user) or printed an error (macOS `fuser` has no `-n tcp`), that number was the port from its `<port>/tcp:` label. Doctor's busy-port check and port release then named, and could offer to stop, a pid equal to the port. Only the pids `fuser` prints on stdout are read now.
- The demo platform's Python services (`identity`, `invoices-api`, `invoices-worker`, `llm`) no longer stall at startup on macOS. Python's `HTTPServer` does a reverse DNS lookup (`socket.getfqdn`) after binding and before it accepts connections; on macOS that lookup can take longer than the 20 s startup timeout, so the service looked hung and failed its health check. The demo servers now skip that lookup. The new macOS CI job caught this in the demo integration test.
- Errors from the supervisor show their hint once. A blocked start printed it twice, as in `web blocked: python3 (pid 8725) is using port 18777 — Open Doctor and press enter to free port 18777, … — Open Doctor and press enter to free port 18777, …`, and so did every other hinted error returned to the CLI or TUI.
- `devctl config import compose` writes published ports in the form the loader reads, `ports: {http: 6379}` with a matching `container.ports`. Before, any service with `ports:` imported as a list that decoded to `port_0: 0`, so validation failed and `--write` refused to save. A port with no host side (`"9000"`) now imports as `auto`, matching Docker's ephemeral host port. Port ranges and `${VAR}` ports are listed under dropped fields instead of being skipped silently. (#135)
- A command array whose items contain `;`, `|`, or `&&`, such as `[node, -e, "a(); b()"]` or `[psql, -c, "select 1; select 2"]`, now validates and runs without `shell: true`. Array items are passed to the process unchanged, so these were never shell syntax. An item that is exactly an operator (`|`, `&&`, `;`, `>`, …) is still rejected, and so is a string command containing `;`, `|`, or `&&`. See [Services](docs/services.md). (#136)
...
- Profiles can **bind named overlays** (`profiles.<name>.environments.<svc>`) and set **per-service env** (`service_environment`) that wins over service vars, so a UI-only profile can point at deployed backends. See [Profiles](docs/profiles.md).
