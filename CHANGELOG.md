...
- Proxy-hop request-id tagging re-emits and persists a reverse-order update, keeps every candidate ingested in the last 50ms of arrival time (matching still uses the 50ms event-time window; folded process lines keep the first line's arrival), and ignores HTTP hop text whose target is not `/…`, an absolute URI, `host:port`, or `*`. TUI live append replaces a repeated positive `seq` inside the same batch.
