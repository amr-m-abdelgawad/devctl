...
- Proxy-hop request-id tagging re-emits and persists a reverse-order update, keeps every candidate still inside the 50ms window behind the newest seen timestamp (not a 64-record cap), and ignores HTTP hop text whose target is not `/…`, an absolute URI, `host:port`, or `*`. TUI live append replaces a repeated positive `seq` inside the same batch.
