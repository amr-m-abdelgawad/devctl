...
- Proxy-hop request-id tagging re-emits and persists a reverse-order update, keeps every candidate still inside the 50ms window (not a 64-record cap), and ignores HTTP hop text whose target is not `/…`, an absolute URI, `host:port`, or `*`.
