...
- Freeing a busy port no longer risks stopping the wrong process. Doctor's free-port action used the process it found when the row was drawn, and sent SIGTERM and then SIGKILL to that pid without checking that it still held the port. If that process had exited and its pid been reused, or it let go of the port on SIGTERM, an unrelated process was signalled. Now devctl looks up the port again before each signal, stops nothing when another process holds it, and reports a port that is already free. Stopping a service that a reload removed from the config also frees its port only when the holder matches the process devctl recorded for it. See [Doctor](docs/doctor.md). (#145)
