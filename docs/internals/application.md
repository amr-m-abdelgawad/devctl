...
2. Store `client_env` per service **only if the request carried one** (MCP/internal starts must not blank a previous real client env). `extra_env` overlays those keys on this start’s launch environment for `req.services` names, or for the full resolved set on a profile-only start, and is not written into the stored `client_env` map.
