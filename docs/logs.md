...
Ingest also copies `devctl.request_id` from a proxy hop onto a nearby service stdout/stderr line that names the same gRPC method or HTTP method+path (50ms event-time match; candidates expire 50ms after ingest arrival so an unrelated future timestamp cannot evict a live pair). HTTP hops require a request-target (`/…`, `http(s)://…`, `host:port`, or `*`). If the service line arrived first, the tagged record is re-emitted and persisted so live views and session reload see the request id. A proxy `caller` attribute, when present, must match the service name. `--dedupe-request-id` (MCP `dedupe_request_id`) then collapses those pairs at query time, keeping the structured proxy attributes and the richer body.
