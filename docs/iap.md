...
User identity mints an ID token with ADC's default OAuth client. To mint with a **specific** OAuth client instead (a Desktop client, or the IAP client's own ID), set `client_id` and `client_secret`. Put `${NAME}` or `${env.NAME}` in `client_secret` (and in `auth.credentials`, `audience`, or extra `auth.headers`) so the value is read from the process environment **or** gitignored `.devctl/secrets.env` (weaker: `~/.devctl/secrets.env`) at mint/request time — that is the same interpolation as service-env `${env.NAME}`. Process env still wins. There is no `${secret:}` syntax:
