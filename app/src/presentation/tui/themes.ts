...
export const THEME_BLURBS: Record<ThemeName, string> = {
  devctl: "forest + mint, website palette",
  ember: "warm orange",
  ...
  voltage: "max contrast, electric neon (dark)",
  "voltage-light": "max contrast, electric neon (light)",
  ironclad: "max contrast, steel & amber (dark)",
  "ironclad-light": "max contrast, steel & amber (light)",
  signal: "max contrast, traffic-light semantics (dark)",
  "signal-light": "max contrast, traffic-light semantics (light)",
...
const PALETTES: Record<ThemeName, Palette> = {
  ...
  // Website brand colors, with brighter foregrounds for terminal readability.
  devctl: {
    name: "devctl",
    primary: "#77DDBA",
    accent: "#9EE9CF",
...
const THEME_ALIASES: Record<string, ThemeName> = {
  ...
  vga: "terminal",
  contrast: "voltage",
  "contrast-light": "voltage-light",
  hc: "voltage",
  "hc-light": "voltage-light",
  a11y: "voltage",
  "a11y-light": "voltage-light",
};
...
export function logSpanColor(palette: Palette, level: string, kind: "text" | "string" | "keyword" | "number" | "search"): string {
  if (kind === "search") {
