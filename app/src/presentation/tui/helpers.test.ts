...
describe("TUI helpers", () => {
  ...
  test("displayLogLevel shows a dash for UNKNOWN and passes every other level through", () => {
    expect(displayLogLevel("UNKNOWN")).toBe("—");
    expect(displayLogLevel("INFO")).toBe("INFO");
    expect(displayLogLevel("ERROR")).toBe("ERROR");
  });
  ...
  test("callerFilterLabel names a service or the no-caller sentinel", () => {
    expect(callerFilterLabel("")).toBe("");
    expect(callerFilterLabel("worker")).toBe("caller: worker");
    expect(callerFilterLabel("-")).toBe("caller: none");
  });
  ...
  test("env table keeps a value column even when keys are long, and flattens multiline values", () => {
    expect(envTableWidths(["PORT"], 40)).toEqual({ key: ENV_KEY_MIN, value: 32 });
    expect(envTableWidths(["GOOGLE_APPLICATION_CREDENTIALS"], 48).key).toBeLessThanOrEqual(ENV_KEY_MAX);
    const tight = envTableWidths(["A".repeat(80)], 20);
    expect(tight.key + tight.value).toBe(20);
    expect(tight.value).toBeGreaterThanOrEqual(ENV_VALUE_MIN);
    expect(envDisplayValue("line1\nline2\tend")).toBe("line1 line2 end");
  });
