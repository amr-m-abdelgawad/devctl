...
// Parallel stacks (#117): list the checkouts holding port slots, and free the
// slots of checkouts that no longer exist.
export function addInstances(root: Command, runtime: ClientRuntime): void {
  const instances = root
    .command("instances")
    .description("list the checkouts running parallel stacks, with their port slots")
...
async function instanceRows(runtime: ClientRuntime): Promise<InstanceRow[]> {
  ...
  for (const entry of runtime.listInstances()) {
    ...
    if (runtime.fileExists(entry.repoRoot)) {
      const client = await runtime.tryDial(entry.repoRoot);
...
export function formatInstances(rows: readonly InstanceRow[]): string {
  if (rows.length === 0) {
    return "no checkouts hold a port slot\n";
  }
  const header = ["SLOT", "OFFSET", "CHECKOUT", "PROXY", "WEB", "OTLP", "STATUS"];
