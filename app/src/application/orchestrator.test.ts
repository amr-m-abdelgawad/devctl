...
function harness(checkers: HealthCheckerFactory = { lookup: () => undefined }) {
  ...
  const session: LifecycleSession = {
    ...
    containerPrefix: "devctl-test-", logs: {
      append: () => {},
      query: () => [],
      queryPage: () => ({ events: [], nextCursor: "", prevCursor: "", hasNext: false, hasPrev: false, sessionChanged: false }),
      queryFacets: () => ({ total: 0, byService: {}, byLevel: {}, bySource: {} }),
      snapshot: () => ({ total: 0, errors: 0, counts: {} }),
      exportTo: () => {},
      setParsers: () => {},
      close: async () => {},
    }, bus: new Bus(32), healthCheckers: checkers,
