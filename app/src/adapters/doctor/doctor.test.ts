...
test("doctor runner preserves attached-service context and diagnostic progress", async () => {
  const host = offlineHost();
  let checkedServicePort = false;
  host.portAvailable = async (port) => {
    if (port === 19991) checkedServicePort = true;
    return true;
  };
  const updates: string[] = [];
  const report = await createDoctorRunner(host).run(localCfg(), (progress) => updates.push(progress.active), {
    services: { api: { pid: 42, ports: { http: 19991 } } },
    repositoryConfigError: "broken local config",
  });
  expect(report.checks.find((check) => check.name === "Port 19991")?.severity).toBe("ok");
