...
describe("supervisor snapshot", () => {
  ...
  test("fail and shutdown cancel pending restart timers immediately", async () => {
    ...
    const internals = (sup as unknown as { orchestrator: { health: {
    ...
    await (sup as unknown as { fail(name: string, err: unknown): Promise<void> }).fail("ghost", new Error("boom"));
...
describe("restart-count bookkeeping", () => {
  test("a manual stop resets the service's restart count", async () => {
    ...
    try {
      await sup.start({ services: ["api"] });
      (sup as unknown as { orchestrator: { health: { bumpRestartCount: (name: string, n: number) => void } } }).orchestrator.health.bumpRestartCount("api", 4);
  ...
  test("an automatic (health-triggered) restart preserves the restart count instead of resetting it", async () => {
    ...
    try {
      await sup.start({ services: ["api"] });
      (sup as unknown as { orchestrator: { health: { bumpRestartCount: (name: string, n: number) => void } } }).orchestrator.health.bumpRestartCount("api", 2);
  ...
  test("sustained healthy operation forgives a service's past restarts", async () => {
    ...
    try {
      await sup.start({ services: ["api"] });
      (sup as unknown as { orchestrator: { health: { bumpRestartCount: (name: string, n: number) => void } } }).orchestrator.health.bumpRestartCount("api", 3);
