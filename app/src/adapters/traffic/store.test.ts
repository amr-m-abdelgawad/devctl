...
describe("TrafficCallRing", () => {
  test("upserts by id, redacts at ingest, and pages newest first", () => {
    ...
    store.upsert([
      ingest("a", { timestamp: "2026-01-01T00:00:01.000Z", request: { text: '{"token":"secret-a"}' } }),
  ...
  test("trims to cap and can install secrets later", () => {
    ...
    store.upsert([ingest("d", { request: { text: '{"token":"still-secret"}' } })]);
    expect(JSON.stringify(store.get("d"))).not.toContain("still-secret");
    store.close();
