...
describe("config allowlist/schema parity", () => {
  test("every object allowlist matches its JSON schema properties", () => {
    ...
    const cases: Array<[string, string[], SchemaNode]> = [
      ...
      ["knownTerraformEnv", knownTerraformEnv, (defs.serviceEnvironment?.properties?.terraform?.oneOf ?? []).find((node) => node.properties) ?? {}],
      ["knownHelmEnv", knownHelmEnv, (defs.serviceEnvironment?.properties?.helm?.oneOf ?? []).find((node) => node.properties) ?? {}],
