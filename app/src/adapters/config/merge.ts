...
function mergeContainer(base: ServiceConfig["container"], raw: unknown): ServiceConfig["container"] {
  ...
  return {
    ...
    seed_from: raw.seed_from !== undefined ? { ...(base?.seed_from ?? {}), ...decoded.seed_from } : (base?.seed_from ?? {}),
    shared_volumes: raw.shared_volumes !== undefined ? decoded.shared_volumes : (base?.shared_volumes ?? []),
...
function mergeServiceOverPresence(base: ServiceConfig, svc: ServiceConfig, present: Set<string>): ServiceConfig {
  ...
  if (present.has("container")) {
    ...
    out.container = container && baseContainer ? {
      ...
      seed_from: present.has("container.seed_from") ? container.seed_from : baseContainer.seed_from,
      shared_volumes: present.has("container.shared_volumes") ? container.shared_volumes : baseContainer.shared_volumes,
