...
`);
    ...
    expect(cfg.services.postgres?.container).toEqual({
      ...
      env: { POSTGRES_PASSWORD: "local" }, volumes: ["pgdata:/var/lib/postgresql/data"], seed_from: {}, shared_volumes: [],
...
`);
    const cfg = load(dir, "");
    expect(cfg.proxy.routes.find((r) => r.name === "api")?.response_headers).toEqual({
      "Access-Control-Allow-Origin": "*",
      "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
    });
  });
